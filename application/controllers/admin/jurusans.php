 ...
 * @author L745
 */
class jurusans extends MY_application_controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('jurusan_model');
        // used in script to determine menu
        $this->load->vars('menu', 'jurusans');
    }

    // for index view
    public function index() {
        $this->is_privilege('INDEX_JURUSAN');
        // pagination
        $this->load->library('pagination');

        // condition for pagination
        if ($this->input->get('column'))
            $data['column'] = in_array(trim($this->input->get('column', TRUE)), $this->jurusan_model->column_names()) ? trim($this->input->get('column', TRUE)) : "nama";
        else
            $data['column'] = "nama";
        ...
        $config['base_url'] = site_url('jurusans?');
        $config['total_rows'] = $this->jurusan_model->get_total($data['cond']);
        $config['per_page'] = '15';
...
//        $page = $this->input->get('per_page', TRUE) ? $this->input->get('per_page', TRUE) : 0;
        ...
        // fetch jurusan
        $data['jurusans'] = $this->jurusan_model->
                fetch_jurusans($config["per_page"], $page, $data['order'], $data['column'], $data['cond']);

        $data['content_title'] = "Data Jurusan";
        $data['breadc'] = array('menu' => "index_jurusan");
        $data['title'] = $this->load->get_var('web_title') . " - " . $data['content_title'];

        $this->load_view('jurusans', 'index', $data);
    }

    // for show view
    public function show($id) {
        $this->is_privilege('SHOW_JURUSAN');
        $data['content_title'] = "Lihat Jurusan";
        $data['breadc'] = array('menu' => "show_jurusan", 'id' => $id);
        $data['title'] = $this->load->get_var('web_title') . " - " . $data['content_title'];
        $data['jurusan'] = $this->jurusan_model->get_jurusan($id);
        if (empty($data['jurusan']))
            show_404();
        $this->load_view('jurusans', 'show', $data);
    }

    // for new view
    public function new_k() {
        $this->is_privilege('NEW_JURUSAN');
        $data['content_title'] = "Buat Data Jurusan";
        $data['breadc'] = array('menu' => "new_jurusan");
        $data['title'] = $this->load->get_var('web_title') . " - " . $data['content_title'];
        ...
        $this->load_view('jurusans', 'new', $data);
    ...
    public function edit($id) {
        $this->is_privilege('EDIT_JURUSAN');
        $data['content_title'] = "Ubah Data Jurusan";
        $data['breadc'] = array('menu' => "edit_jurusan", 'id' => $id);
        $data['title'] = $this->load->get_var('web_title') . " - " . $data['content_title'];
        $data['jurusan'] = $this->jurusan_model->get_jurusan($id);
        if (empty($data['jurusan']))
            show_404();
        ...
        $data['jurusan']['nama'] = isset($this->session->userdata('field')['nama']) ? 
                $this->session->userdata('field')['nama'] : $data['jurusan']['nama'];
        
        $this->load_view('jurusans', 'edit', $data);
        
        // unset session field data when error from method create
        $this->session->unset_userdata('field');
    }

    // for create new jurusan
    public function create() {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($this->jurusan_model->create($data)) {
                $this->session->set_flashdata('message',
                        '<div class="alert alert-success">'.
                        '<a class="close" data-dismiss="alert" href="#">&times;</a>'.
                        'Data jurusan berhasil ditambah</div>');
                redirect('jurusans');
            }
        }
    ...
    public function update($id) {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($this->jurusan_model->update($id, $data)) {
                $this->session->set_flashdata('message', 
                        '<div class="alert alert-success">'.
                        '<a class="close" data-dismiss="alert" href="#">&times;</a>'.
                        'Data jurusan berhasil diubah</div>');
                redirect('jurusans/' . $id);
            }
        }
        ...
        redirect('jurusans/' . $id . '/edit');
    }

    // for delete jurusan with ID
    public function delete($id) {
        $this->is_privilege('DELETE_JURUSAN');
        if ($this->jurusan_model->delete($id)) {
            ...
            $this->session->set_flashdata('message', '<div class="alert">'.
                    '<a class="close" data-dismiss="alert" href="#">&times;</a>'.
                    'Data jurusan gagal dihapus</div>');
        }
        redirect('jurusans');
    }

    // for delete jurusan with multiple ID
    public function deletes() {
        $this->is_privilege('DELETE_JURUSAN');
        $affected_row = $this->input->post('ids', TRUE) ? $this->jurusan_model->deletes($this->input->post('ids', TRUE)) : 0;
        if($affected_row){
            $this->session->set_flashdata('message',
                '<div class="alert alert-info">'.
    ...
    public function nama_check($str) {
        $query = $this->jurusan_model->unique_check(trim($str));
        if ($query->num_rows() > 0) {
