...
<?php echo form_open('gurumatapelajarans/'.$guru_mata_pelajaran['id'].'/update', 'class="form-horizontal"') ?>
...
<div class="control-group <?php if(($this->session->flashdata('guru_id'))) echo 'error'; ?>">
    ...
    <div class="controls">
        ...
        <input type="hidden" name="guru_id" value="<?php echo $guru_mata_pelajaran['guru_id'] ?>">
    </div>
</div>
        ...
        <select class="span2" name="mata_pelajaran_id">
            <?php if (!empty($mata_pelajarans)) { ?>
                <?php foreach ($mata_pelajarans as $mata_pelajaran): ?>
                    <option value="<?php echo $mata_pelajaran['id'] ?>" <?php echo $guru_mata_pelajaran['mata_pelajaran_id'] === $mata_pelajaran['id'] ? "selected" : "" ?>><?php echo get_full_mata_pelajaran($mata_pelajaran['id']) ?></option>
                <?php endforeach; ?>
            <?php }else { ?>
                    <option value=""> </option>
            <?php } ?>
...
<div class="control-group <?php if (($this->session->flashdata('semester'))) echo 'error'; ?>">
    ...
    <div class="controls">
        <select class="span2" name="semester">
            <option value="1" <?php echo $guru_mata_pelajaran['semester'] === "1" ? "selected" : "" ?>>Ganjil</option>
            <option value="2" <?php echo $guru_mata_pelajaran['semester'] === "2" ? "selected" : "" ?>>Genap</option>
        </select>
    </div>
</div>
        ...
        <select class="span2" name="kurikulum_id">
            <?php if (!empty($kurikulums)) { ?>
                <?php foreach ($kurikulums as $kurikulum): ?>
                    <option value="<?php echo $kurikulum['id'] ?>" <?php echo $guru_mata_pelajaran['kurikulum_id'] === $kurikulum['id'] ? "selected" : "" ?>><?php echo get_nama_kurikulum($kurikulum['id']) ?></option>
                <?php endforeach; ?>
            <?php }else { ?>
                    <option value=""> </option>
            <?php } ?>
        ...
        <select class="span2" name="tahun_ajaran_id">
            <?php if (!empty($tahun_ajarans)) { ?>
                <?php foreach ($tahun_ajarans as $tahun_ajaran): ?>
                    <option value="<?php echo $tahun_ajaran['id'] ?>" <?php echo $guru_mata_pelajaran['tahun_ajaran_id'] === $tahun_ajaran['id'] ? "selected" : "" ?>><?php echo get_nama_tahun_ajaran($tahun_ajaran['id']) ?></option>
                <?php endforeach; ?>
            <?php }else { ?>
                    <option value=""> </option>
            <?php } ?>
...
<div class=" form-actions">
    ...
    <a href="<?php echo site_url('gurus/'.$guru_mata_pelajaran['guru_id']) ?>" class="btn">Kembali</a>
</div>
</form>
