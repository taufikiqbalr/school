...
<?php echo form_open('siswakelas/'.$siswa_kelas['id'].'/update', 'class="form-horizontal"') ?>
...
<div class="control-group <?php if (($this->session->flashdata('siswa_id'))) echo 'error'; ?>">
    ...
    <div class="controls">
        <label class="checkbox" for="siswa_id"><?php echo $siswa['nama'] ?></label>
        <input type="hidden" name="siswa_id" value="<?php echo $siswa_kelas['siswa_id'] ?>">
    </div>
</div>
<div class="control-group <?php if (($this->session->flashdata('kelas_bagian_id'))) echo 'error'; ?>">
    ...
    <div class="controls">
        <select class="span2" name="kelas_bagian_id">
            ...
            <option value="<?php echo $kelas_bagian['id'] ?>" <?php echo $siswa_kelas['kelas_bagian_id'] === $kelas_bagian['id'] ? "selected" : "" ?>><?php echo get_full_kelas($kelas_bagian['id']) ?></option>
                <?php endforeach; ?>
            <?php }else { ?>
                    <option value=""> </option>
            <?php } ?>
        ...
        <select class="span2" name="tahun_ajaran_id">
            <?php if (!empty($tahun_ajarans)) { ?>
                <?php foreach ($tahun_ajarans as $tahun_ajaran): ?>
                    <option value="<?php echo $tahun_ajaran['id'] ?>" <?php echo $siswa_kelas['tahun_ajaran_id'] === $tahun_ajaran['id'] ? "selected" : "" ?>><?php echo get_nama_tahun_ajaran($tahun_ajaran['id']) ?></option>
                <?php endforeach; ?>
            <?php }else { ?>
                    <option value=""> </option>
            <?php } ?>
...
<div class=" form-actions">
    ...
    <a href="<?php echo site_url('siswas/'.$siswa_kelas['siswa_id']) ?>" class="btn">Kembali</a>
</div>
</form>