...
<form id="form-spp-edit" class="form-horizontal" accept-charset="utf-8" method="post" action="<?php echo base_url() . 'spps/' . $spp['id'] . '/update' ?>">
            ...
            <select class="span3" name="siswa_id">
                ...
                <?php if (!empty($siswas)) { ?>
                    <?php foreach ($siswas as $siswa): ?>
                        <option value="<?php echo $siswa['id'] ?>" <?php echo $spp['siswa_id'] === $siswa['id'] ? "selected" : "" ?>><?php echo $siswa['nis'] . '-' . $siswa['nama'] ?></option>
                    <?php endforeach; ?>
                <?php } ?>
            </select>
        </div>
        ...
        <div class="controls">
            <select name="bulan_tempo">
                ...
                <?php foreach (months() as $key => $month) : ?>
                    <option value="<?php echo $key + 1 ?>" <?php echo ($key + 1) === ((int) $spp['bulan_tempo']) ? "selected" : "" ?>><?php echo $month ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    </div>
    <div class="control-group <?php if (($this->session->flashdata('tahun_tempo'))) echo 'error'; ?>">
        ...
        <div class="controls">
            <input autocomplete="off" class="typeahead-tahun" type="text" name="tahun_tempo" placeholder="Tahun..." value="<?php echo $spp['tahun_tempo'] ?>">
        </div>
    </div>
    <div class="control-group <?php if (($this->session->flashdata('biaya'))) echo 'error'; ?>">
        ...
        <div class="controls">
            <div class="input-prepend input-append">
                ...
                <input name="biaya" class="positive-integer" type="text" placeholder="Biaya..." value="<?php echo $spp['biaya'] ?>">
    ...
    <div class="control-group <?php if (($this->session->flashdata('bayar'))) echo 'error'; ?>">
        ...
        <div class="controls">
            <div class="input-prepend input-append">
                <span class="add-on">Rp</span>
                <input name="bayar" class="positive-integer" type="text" placeholder="Bayar..." value="<?php echo $spp['bayar'] ?>">
                <span class="add-on">.00</span>
            </div>
        </div>
    </div>
    <div class="control-group <?php if (($this->session->flashdata('tanggal_bayar'))) echo 'error'; ?>">
        ...
        <div class="controls">
            <div id="dp-spp-new-tanggal_bayar" class="input-append date" data-date-format="yyyy-mm-dd" data-date="2012-12-12">
                <input name="tanggal_bayar" class="span2" type="text" readonly="" value="<?php echo $spp['tanggal_bayar'] ?>">
                <span class="add-on">
    ...
    <div class=" form-actions">
        ...
        <a href="<?php echo site_url('spps/' . $spp['id']) ?>" class="btn">Kembali</a>
    </div>
</form>
