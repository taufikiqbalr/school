...
<?php echo form_open('guruwalis/' . $guru_wali['id'] . '/update', 'class="form-horizontal"') ?>
...
<div class="control-group <?php if (($this->session->flashdata('guru_id'))) echo 'error'; ?>">
    ...
    <div class="controls">
        ...
        <input type="hidden" name="guru_id" value="<?php echo $guru_wali['guru_id'] ?>">
    </div>
</div>
        ...
        <select class="span2" name="kelas_bagian_id">
            <?php if (!empty($kelas_bagians)) { ?>
                <?php foreach ($kelas_bagians as $kelas_bagian): ?>
                    <option value="<?php echo $kelas_bagian['id'] ?>" <?php echo $guru_wali['kelas_bagian_id'] === $kelas_bagian['id'] ? "selected" : "" ?>><?php echo get_full_kelas($kelas_bagian['id']) ?></option>
                <?php endforeach; ?>
            <?php }else { ?>
                    <option value=""> </option>
            <?php } ?>
        ...
        <select class="span2" name="tahun_ajaran_id">
            <?php if (!empty($tahun_ajarans)) { ?>
                <?php foreach ($tahun_ajarans as $tahun_ajaran): ?>
                    <option value="<?php echo $tahun_ajaran['id'] ?>" <?php echo $guru_wali['tahun_ajaran_id'] === $tahun_ajaran['id'] ? "selected" : "" ?>><?php echo get_nama_tahun_ajaran($tahun_ajaran['id']) ?></option>
                <?php endforeach; ?>
            <?php }else { ?>
                    <option value=""> </option>
            <?php } ?>
...
<div class=" form-actions">
    ...
    <a href="<?php echo site_url('gurus/' . $guru_wali['guru_id']) ?>" class="btn">Kembali</a>
</div>
</form>
