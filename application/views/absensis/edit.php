...
<?php echo form_open('absensis/'.$absensi['id'].'/update', 'class="form-horizontal"') ?>
        ...
        <select class="span3" name="siswa_id">
            ...
            <?php if (!empty($siswas)) { ?>
                <?php foreach ($siswas as $siswa): ?>
                    <option value="<?php echo $siswa['id'] ?>" <?php echo $absensi['siswa_id'] === $siswa['id'] ? "selected" : "" ?>><?php echo $siswa['nis'] . '-' . $siswa['nama'] ?></option>
                <?php endforeach; ?>
            <?php } ?>
        </select>
    </div>
        ...
        <select class="span3" name="kelas_bagian_id">
            ...
            <?php if (!empty($kelas_bagians)) { ?>
                <?php foreach ($kelas_bagians as $kelas_bagian): ?>
                    <option value="<?php echo $kelas_bagian['id'] ?>" <?php echo $absensi['kelas_bagian_id'] === $kelas_bagian['id'] ? "selected" : "" ?>><?php echo get_full_kelas($kelas_bagian['id']) ?></option>
                <?php endforeach; ?>
            <?php } ?>
        </select>
    </div>
        ...
        <select class="span3" name="mata_pelajaran_id">
            ...
            <?php if (!empty($mata_pelajarans)) { ?>
                <?php foreach ($mata_pelajarans as $mata_pelajaran): ?>
                    <option value="<?php echo $mata_pelajaran['id'] ?>" <?php echo $absensi['mata_pelajaran_id'] === $mata_pelajaran['id'] ? "selected" : "" ?>><?php echo get_full_mata_pelajaran($mata_pelajaran['id']) ?></option>
                <?php endforeach; ?>
            <?php } ?>
        </select>
    </div>
...
<div class="control-group <?php if (($this->session->flashdata('guru_id'))) echo 'error'; ?>">
    ...
    <div class="controls">
        <select class="span3" name="guru_id">
            ...
            <option value="<?php echo $guru['id'] ?>" <?php echo $absensi['guru_id'] === $guru['id'] ? "selected" : "" ?>><?php echo get_full_guru($guru['id']) ?></option>
                <?php endforeach; ?>
            <?php } ?>
        </select>
    </div>
...
<div class="control-group <?php if (($this->session->flashdata('tanggal'))) echo 'error'; ?>">
    ...
    <div class="controls">
        <select name="tanggal">
            <option value=""> </option>
            <?php for ($i = 1; $i <= 31; $i++) { ?>
                <option value="<?php echo $i ?>" <?php echo ((int)$absensi['tanggal']) === $i ? "selected" : "" ?>><?php echo $i ?></option>
            <?php } ?>
        </select>
    </div>
</div>
    ...
    <div class="controls">
        <select name="bulan">
            ...
            <?php foreach (months() as $key => $month) : ?>
                <option value="<?php echo $key + 1 ?>" <?php echo ((int)$absensi['bulan']) === ($key+1) ? "selected" : "" ?>><?php echo $month ?></option>
            <?php endforeach; ?>
        </select>
    </div>
</div>
        ...
        <select name="tahun_ajaran_id">
            ...
            <?php if (!empty($tahun_ajarans)) { ?>
                <?php foreach ($tahun_ajarans as $tahun_ajaran): ?>
                    <option value="<?php echo $tahun_ajaran['id'] ?>" <?php echo $absensi['tahun_ajaran_id'] === $tahun_ajaran['id'] ? "selected" : "" ?>><?php echo $tahun_ajaran['nama'] ?></option>
                <?php endforeach; ?>
            <?php } ?>
        </select>
    </div>
    ...
    <div class="controls">
        <select name="absensi">
            ...
            <?php foreach (absensi() as $key => $absen) : ?>
                <option value="<?php echo $key+1 ?>" <?php echo ((int)$absensi['absensi']) === ($key+1) ? "selected" : "" ?>><?php echo $absen ?></option>
            <?php endforeach; ?>
        </select>
    </div>
</div>
<div class="control-group <?php if (($this->session->flashdata('keterangan'))) echo 'error'; ?>">
    ...
    <div class="controls">
        <input type="text" name="keterangan" placeholder="Keterangan..." value="<?php echo $absensi['keterangan'] ?>">
    </div>
</div>
<div class=" form-actions">
    <button type="submit" name="submit" class="btn btn-success button-save" data-loading-text="Menyimpan...">Simpan</button>
    <a href="<?php echo site_url('absensis/'.$absensi['id']) ?>" class="btn">Kembali</a>
</div>
</form>