...
class user_model extends CI_Model {
    ...
    public function column_names(){
        return $this->db->list_fields('user_accounts');
    }
    
    // get user with ID
    public function get_user($id = FALSE) {
        $this->db->join('user_groups', 'user_groups.ugrp_id = user_accounts.uacc_group_fk', 'left');
        if ($id === FALSE) {
            $query = $this->db->get('user_accounts');
            return $query->result_array();
        }
        $query = $this->db->get_where('user_accounts', array('uacc_id' => $id));
    ...
    public function get_total($cond = FALSE) {
        if(!empty($cond)) {
            $this->db->like('uacc_username',$cond);
        }
        return $this->db->count_all_results('user_accounts');
    }
    
    // get jurusan for pagination
    public function fetch_users($limit, $start, $order = FALSE, $field = FALSE, $cond = FALSE) {
        $this->db->join('user_groups', 'user_groups.ugrp_id = user_accounts.uacc_group_fk', 'left');
    ...
    public function delete($id) {
        $query = $this->db->get_where("user_accounts", array('uacc_id' => trim($id)));
        if ($query->num_rows() > 0) {
    ...
    public function deletes($ids) {
        if (!empty($ids)) {
            $this->db->query("DELETE FROM user_accounts WHERE uacc_id IN (".implode(',',$ids).");");
